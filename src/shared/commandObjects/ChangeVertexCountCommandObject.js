...
export default class ChangeVertexCountCommandObject extends CommandObject {
  constructor(undoHandler, data, isRepeat) {
    ...
    this.newValue = data.newValue; // number
    this.oldValue = data.oldValue; // number
    this.commandName = `${isRepeat ? `[Repeat] ` : ''}Change ${
      this.targetObject.type
    } Vertex Count to ${this.newValue}`;
    this.repeatAction = undoHandler.changeCurrVertexCount;
  }
   ...
   */
  undo() {
    this.undoHandler.updateShape(this.targetObject.id, { vertexCount: this.oldValue });
    this.undoHandler.selectShape(this.targetObject.id, { vertexCount: this.oldValue });
  }
   ...
   */
  redo() {
    this.undoHandler.updateShape(this.targetObject.id, { vertexCount: this.newValue });
    this.undoHandler.selectShape(this.targetObject.id, { vertexCount: this.newValue });
  }
  ...
  canRepeat() {
    ...
    if (
      ...
      currentWorkspaceShape.id === commamndShape.id ||
      currentWorkspaceShape.type !== 'polygon'
    ) {
      return false;
    } else {
      return true;
